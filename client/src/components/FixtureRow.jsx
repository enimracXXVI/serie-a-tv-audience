...
function AudienceInput({ value, placeholder, onCommit, canEdit }) {
  ...
  return (
    <input
      ...
      inputMode="decimal"
      disabled={!canEdit}
      className="score-input w-16 rounded-md border border-white/10 bg-white/[0.04] px-1.5 py-1 text-center text-xs text-white/90 outline-none transition-colors focus:border-white/40 placeholder:text-white/25 disabled:cursor-not-allowed disabled:opacity-60"
...
function ScoreInput({ value, onCommit, canEdit }) {
  const [draft, setDraft] = useState(value ?? '');
  return (
    <input
      type="number"
      ...
      max="20"
      disabled={!canEdit}
      className="score-input h-7 w-8 rounded-md border border-white/15 bg-white/[0.06] text-center text-sm font-bold text-white outline-none focus:border-white/50 disabled:cursor-not-allowed disabled:opacity-60"
      placeholder="-"
      value={draft}
      onChange={(e) => setDraft(e.target.value)}
      onBlur={() => {
...
export default function FixtureRow({ fixture, onUpdate, highlightSlugs = [], canEdit, onRequireSignIn }) {
  ...
  const gate = (children) =>
    canEdit ? (
      ...
      <div onClick={onRequireSignIn} className="cursor-pointer" title="Sign in to edit">
        {children}
      </div>
    );
      ...
      <div className="flex flex-col items-center gap-1.5">
        {gate(
          <div className="flex items-center gap-1.5">
            <ScoreInput canEdit={canEdit} value={fixture.homeScore} onCommit={(v) => onUpdate(fixture.id, { homeScore: v })} />
            <span className="text-white/30 text-xs">-</span>
            <ScoreInput canEdit={canEdit} value={fixture.awayScore} onCommit={(v) => onUpdate(fixture.id, { awayScore: v })} />
          </div>
        )}
        {gate(
