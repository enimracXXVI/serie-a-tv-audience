...
export default function FixtureSearch({ fixtures, onSelect }) {
  ...
  const results = useMemo(() => searchFixtures(fixtures, query).slice(0, 8), [fixtures, query]);
  ...
  return (
    <div ref={ref} className="relative min-w-0 flex-1">
      ...
      {open && query && (
        <div className="absolute left-0 right-0 z-40 mt-1 max-h-80 overflow-y-auto rounded-lg bg-[#0f1e54] py-1 shadow-xl ring-1 ring-white/10">
          {results.length === 0 ? (
